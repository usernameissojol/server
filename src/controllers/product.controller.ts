...
const safelyParseJSON = (data: any, fallback: any = []) => {
    ...
    try {
        ...
        const parsedProducts = products.map((p: any) => {
            ...
            const tags = Array.isArray(p.product_tags) && p.product_tags.length > 0
                ? p.product_tags.map((pt: any) => pt.tags?.name).filter(Boolean)
...
export const getProductById = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const tags = p.product_tags?.map((pt: any) => pt.tags?.name).filter(Boolean) || [];
...
export const bulkUpdateProducts = async (req: Request, res: Response) => {
    try {
        ...
        if (data.category_id) updateData.category_id = parseInt(data.category_id);
        if (data.brand_id) updateData.brand_id = parseInt(data.brand_id);
            ...
            try {
                ...
                if (!categoryId && categoryName) {
                    ...
                    if (categoryCache.has(cleanCat)) {
                        ...
                        let category = await prisma.categories.findFirst({ where: { name: cleanCat, parent_id: null } });
                        if (!category) {
                    ...
                    if (subCatName) {
                        ...
                        let subCategory = await prisma.categories.findFirst({ where: { name: cleanSub, parent_id: parentId } });
                ...
                if (!brandId && brandName) {
                    ...
                    if (brandCache.has(cleanBrand)) {
                        ...
                        let brand = await prisma.brands.findFirst({ where: { name: cleanBrand } });
                        if (!brand) {
